...
std::string prepareOutputFileName(const std::string &configFile,
                                  const std::string &outputFileNameFromCfg) {
...
SceneWidget::SceneWidget(QWidget *parent)
    : QVTKOpenGLNativeWidget(parent),
      sceneWidgetVisualizerProxy{
          SceneWidgetVisualizerFactory::create(ModelType::Ball)},
  ...
  connect(&ColorSettings::instance(), &ColorSettings::colorsChanged, this,
          &SceneWidget::onColorsReloadRequested);
}
...
void SceneWidget::loadAndUpdateVisualizationForCurrentStep() {
  ...
  sceneWidgetVisualizerProxy->readStageStateFromFilesForStep(
      settingParameter.get(), &lines[0]);

  // Refresh VTK visualization elements
  sceneWidgetVisualizerProxy->refreshWindowsVTK(
      settingParameter->numberOfRowsY, settingParameter->numberOfColumnX,
      settingParameter->step, &lines[0], settingParameter->numberOfLines,
      gridActor);

  // visualiserProxy->vis->refreshBuildLoadBalanceLine(lines,
  // cam->numberOfLines, cam->dimY+1, cam->dimX+1, actorBuildLine, colors, pts,
  // cellLines, grid);
...
void SceneWidget::setupSettingParameters(const std::string &configFilename,
  ...
  // Plus additional lines for bottom edge (nNodeX lines) and right edge (nNodeY
  // lines)
  settingParameter->numberOfLines =
      2 * (settingParameter->nNodeX * settingParameter->nNodeY) +
      settingParameter->nNodeX + settingParameter->nNodeY;
  settingParameter->step = stepNumber;
  settingParameter->changed = false;

  sceneWidgetVisualizerProxy->initMatrix(settingParameter->numberOfColumnX,
...
void SceneWidget::readSettingsFromConfigFile(const std::string &filename) {
...

  {
    ...
    const std::string outputFileNameFromCfg =
        generalContext->getConfigParameter("output_file_name")
            ->getValue<std::string>();
    settingParameter->outputFileName =
        prepareOutputFileName(filename, outputFileNameFromCfg);
    settingParameter->numberOfColumnX =
        generalContext->getConfigParameter("number_of_columns")
            ->getValue<int>();
    settingParameter->numberOfRowsY =
        generalContext->getConfigParameter("number_of_rows")->getValue<int>();
    settingParameter->nsteps =
        generalContext->getConfigParameter("number_steps")->getValue<int>();
...

  {
    ...
    settingParameter->nNodeX =
        execContext->getConfigParameter("number_node_x")->getValue<int>();
    settingParameter->nNodeY =
        execContext->getConfigParameter("number_node_y")->getValue<int>();
    /// Notice: there are much more params, which are not used: e.g.
...
void SceneWidget::setupVtkScene() {
  ...
  renderWindow()->SetSize(settingParameter->numberOfColumnX,
                          settingParameter->numberOfRowsY + 10);
  ...
  renderWindow()->SetWindowName(
      QApplication::applicationName().toLocal8Bit().data());
...
void SceneWidget::setup2DRulerAxes() {
  ...
  rulerAxisY->SetTitlePosition(
      1.2); // Move title further from axis (default is ~0.5)
...
void SceneWidget::update2DRulerAxesBounds() {
  ...
  // Position Y axis at the RIGHT of the data (vertical line) - labels won't
  // overlap scene
...
void SceneWidget::keypressCallbackFunction(vtkObject *caller,
  ...
  if (sp->changed) {
    try {
      ...
      std::cerr << "Runtime error di getElementMatrix: " << re.what()
                << std::endl;
...
void SceneWidget::cameraCallbackFunction(vtkObject *caller,
                                         long unsigned int eventId,
                                         void *clientData, void *callData) {
  Q_UNUSED(caller);
  Q_UNUSED(eventId);
  ...
  if (self->currentViewMode == ViewMode::Mode3D && self->renderer) {
    ...
    if (camera) {
      ...
      double elevation =
          std::atan2(dz, std::sqrt(dx * dx + dy * dy)) * 180.0 / vtkMath::Pi();
...
void SceneWidget::mouseCallbackFunction(vtkObject *caller,
                                        long unsigned int eventId,
                                        void *clientData, void *callData) {
  ...
  // 3) Use a picker to obtain an accurate world position (if something was
  // "hit")
  vtkNew<vtkPropPicker> picker;
  bool picked = false;
  if (self->renderer) {
    // Pick returns 1 if something was hit (depending on the picker). Pass the
    // renderer.
  ...
  if (!picked && self->renderer) {
    double displayPt[3] = {static_cast<double>(vtkX), static_cast<double>(vtkY),
                           0.0};
    ...
    if (worldPt[3] != 0.0) {
      self->m_lastWorldPos = {worldPt[0] / worldPt[3], worldPt[1] / worldPt[3],
                              worldPt[2] / worldPt[3]};
    } else {
...
void SceneWidget::renderVtkScene() {
  ...
  emit availableStepsReadFromConfigFile(
      sceneWidgetVisualizerProxy->availableSteps());

  lines.resize(settingParameter->numberOfLines);
  sceneWidgetVisualizerProxy->readStageStateFromFilesForStep(
      settingParameter.get(), &lines[0]);

  sceneWidgetVisualizerProxy->drawWithVTK(
      settingParameter->numberOfRowsY, settingParameter->numberOfColumnX,
      settingParameter->step, renderer, gridActor);

  sceneWidgetVisualizerProxy->getVisualizer().buildLoadBalanceLine(
      lines, settingParameter->numberOfColumnX + 1, renderer, actorBuildLine);

  sceneWidgetVisualizerProxy->getVisualizer().buildStepText(
      settingParameter->step, settingParameter->font_size, singleLineTextStep,
      renderer);
...
std::array<double, 3>
SceneWidget::screenToWorldCoordinates(const QPoint &pos) const {
...
QString SceneWidget::getNodeAtWorldPosition(
    const std::array<double, 3> &worldPos) const {
  ...
  if (nodeX >= 0 && nodeX < settingParameter->nNodeX && nodeY >= 0 &&
      nodeY < settingParameter->nNodeY) {
...
const Line *SceneWidget::findNearestLine(const std::array<double, 3> &worldPos,
                                         size_t &lineIndex,
  ...
  constexpr double threshold =
      2; // Threshold for line selection (in world coordinates)
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    const double t = std::max(
        0.0, std::min(1.0, ((worldPos[0] - line.x1) * (line.x2 - line.x1) +
                            (worldPos[1] - line.y1) * (line.y2 - line.y1)) /
                               lineLengthSq));
...
void SceneWidget::updateToolTip(const QPoint &lastMousePos) {
  ...
  // m_lastWorldPos is set by the VTK callback (picker or DisplayToWorld
  // fallback)
  ...
  const Line *nearestLine =
      findNearestLine(m_lastWorldPos, lineIndex, distanceSq);
  ...
  } else if (QString nodeInfo = getNodeAtWorldPosition(m_lastWorldPos);
             !nodeInfo.isEmpty()) {
  ...
  QToolTip::showText(globalPos, tooltipText, this,
                     QRect(lastMousePos, QSize(1, 1)), 0);
}
...
void SceneWidget::switchModel(ModelType modelType) {
  ...
  std::cout << "Switched to model: "
            << sceneWidgetVisualizerProxy->getModelName() << std::endl;
}
...
void SceneWidget::loadNewConfiguration(const std::string &configFileName,
                                       int stepNumber) {
  try {
