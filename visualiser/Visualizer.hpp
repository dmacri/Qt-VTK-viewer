...
public:
  ...
  void drawWithVTK(/*const*/ Matrix &p, int nRows, int nCols, StepIndex step,
                   vtkSmartPointer<vtkRenderer> renderer,
                   vtkSmartPointer<vtkActor> gridActor);
  template <class Matrix>
  void refreshWindowsVTK(/*const*/ Matrix &p, int nRows, int nCols,
                         StepIndex step, Line *lines, int dimLines,
                         vtkSmartPointer<vtkActor> gridActor);
  void buildLoadBalanceLine(const std::vector<Line> &lines, int nCols,
                            vtkSmartPointer<vtkRenderer> renderer,
                            vtkSmartPointer<vtkActor2D> actorBuildLine);
  void refreshBuildLoadBalanceLine(Line *lines, int dimLines, int nCols,
                                   int nRows, vtkActor2D *lineActor);
  vtkTextProperty *buildStepLine(StepIndex step,
                                 vtkSmartPointer<vtkTextMapper>);
  vtkNew<vtkActor2D>
  buildStepText(StepIndex step, int font_size,
...
template <class Matrix>
void Visualizer::drawWithVTK(/*const*/ Matrix &p, int nRows, int nCols,
                             StepIndex step,
                             vtkSmartPointer<vtkRenderer> renderer,
                             vtkSmartPointer<vtkActor> gridActor) {
...
template <class Matrix>
void Visualizer::refreshWindowsVTK(/*const*/ Matrix &p, int nRows, int nCols,
                                   StepIndex step, Line *lines, int dimLines,
                                   vtkSmartPointer<vtkActor> gridActor) {
  if (vtkLookupTable *lut = dynamic_cast<vtkLookupTable *>(
          gridActor->GetMapper()->GetLookupTable())) {
    buidColor(lut, nCols, nRows, p);
 ...
 *
 * This function is designed to be forward-compatible with upcoming changes in
 * OOpenCal. Currently, OOpenCal provides color values in the 0–255 integer
 * range. VTK, however, expects normalized double precision values in the 0–1
 * range.
 *
 * If the input value is greater than 1, it is assumed to be in the 0–255 range
 * and will be scaled down to [0, 1]. If the value is already in the [0, 1]
 * range (future OOpenCal format), it will be returned unchanged.
 *
...
template <class Matrix>
void Visualizer::buidColor(vtkLookupTable *lut, int nCols, int nRows,
                           Matrix &p) {
  for (int r = 0; r < nRows; ++r) {
    for (int c = 0; c < nCols; ++c) {
      rgb *color = p[r][c].outputValue(nullptr);
      lut->SetTableValue(
          (nRows - 1 - r) * nCols + c, toUnitColor(color->getRed()),
          toUnitColor(color->getGreen()), toUnitColor(color->getBlue()),
